...
datfiles = {};
for gcwind = -4:4;
    df = dir(sprintf('avar_dat_%s_%s_%.0f_*.mat',noise_sta_deets.sta,noise_sta_deets.nwk,noise_sta_deets.gc+gcwind));
...
for id = 1:length(dtypes)
    ...
    ip = find(strcmp(dtype(1:2),avar.phases));
    ...
    for ia = 1:length(trudata.(dtype))
        ...
        ndat=ndat./sqrt(mean(ndat.^2)); %norm
        %% spectrum
        nw=4;
        ...
        % assemble final data
        odat(:,ic) = idat + rndat/ampfac;
        
        % posterior guess at SNR
%         inoise = trudata.(dtype)(ia).tt>=-50 & trudata.(dtype)(ia).tt<-25;
%         iSNR(jj,ic) = max(odat(:,ic))./2./rms(odat(inoise,ic));
